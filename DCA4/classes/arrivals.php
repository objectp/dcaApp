...
class Arrivals{
    ...
    public function __construct(){
        ...
        $this->codesAndTerminals = self::assignCodesToTerminals();
        $this->codesTerminalsTimes = self::combineCodeTerminalTime();
        $this->timeElapses = self::findElaspe();
    ...
    public function combineCodeTerminalTime(){
       return array_combine($this->codesAndTerminals, $this->flightTimes); 
    }
    //Get each url
    ...
    public function assignCodesToTerminals(){
    ...
    function findElaspe(){
        $arrivalTimes = self::getArrivalsTimes();
        
        foreach($arrivalTimes as $t){
           $tString = self::dateToString($t);
           $oldTime = strtotime($tString);
           $newTime = time();
           $timeAgo = self::timeElapse($newTime - $oldTime);
           $resultArray[] = $timeAgo;
        }
...
// convert to json
$json = json_encode($arrivalsObject);
